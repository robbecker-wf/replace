...
Future<int> _run(List<String> args) async {
	...
	for (final path in pubspecFiles) {
		...
		if (targetsSdk) {
			updateMessages.addAll(_applyToSdk(pubspec, op, versionText));
		} else {
			updateMessages.addAll(
				_applyToDependencies(pubspec.dependencies, dependencyName, op, versionText),
			);
			updateMessages.addAll(
...
List<String> _applyToDependencies(
	...
	if (!nextBounds.isValid()) {
		stderr.writeln(
			'Skipping ${dependency.name}: resulting range would be empty (${nextBounds.toConstraintString()})',
		);
		return const [];
	}

	final nextText = _toYamlConstraint(
		nextBounds.toConstraintString(),
		currentText,
	);
	if (nextText == currentText) {
		return const [];
...
List<String> _applyToSdk(
	...
	if (!nextBounds.isValid()) {
		stderr.writeln(
			'Skipping sdk: resulting range would be empty (${nextBounds.toConstraintString()})',
		);
		return const [];
	}

	final nextText = _toYamlConstraint(nextBounds.toConstraintString(), currentText);
	if (nextText == currentText) {
		return const [];
	}
...
ArgParser _buildParser() {
	final parser = ArgParser()
		...
		..addFlag(
			'fail-on-parse-error',
			negatable: false,
			help: 'If a pubspec.yaml cannot be parsed, fail immediately and set the exit code',
		)
		..addFlag(
			'recursive',
			abbr: 'r',
...
void _printUsage(ArgParser parser) {
  ...
  stdout.writeln('  set         Set the version of a dependency');
	stdout.writeln('  lower-max   Lower the maximum allowed version (exclusive) of a dependency.');
	stdout.writeln('  raise-max   Raise the maximum allowed version (exclusive) of a dependency.');
  stdout.writeln('  raise-min   Raise the minimum allowed version (inclusive) of a dependency.');
  stdout.writeln('(sdk)');
  stdout.writeln('  set-sdk     Set the SDK version constraint in environment.sdk.');
	stdout.writeln('  raise-max-sdk Raise the maximum allowed SDK version (exclusive) in environment.sdk.');
	stdout.writeln('  raise-min-sdk Raise the minimum allowed SDK version (inclusive) in environment.sdk.');
}
...
class _ConstraintBounds {
	...
	static _ConstraintBounds? fromConstraint(semver.VersionConstraint constraint) {
...
String _toYamlConstraint(String value, String existingValue) {
	...
	final useDoubleQuote =
			existingTrimmed.length >= 2 &&
